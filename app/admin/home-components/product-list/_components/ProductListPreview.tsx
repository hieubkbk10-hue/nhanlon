...
export const ProductListPreview = ({
  ...
  const renderSectionHeader = (opts?: { className?: string }) => {
    ...
    if (headerRightSlot) {
      return (
        <div className={cn('mb-6 md:mb-8', opts?.className)}>
          <SectionHeader
            title={displayTitle}
            subtitle={displaySubtitle}
            badgeText={subTitle}
      ...
      <div className={cn(
        ...
        {displayItems.slice(0, device === 'mobile' ? 4 : (device === 'tablet' ? 6 : itemCount)).map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="group bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col cursor-pointer"
            >
    ...
    <section className={cn("py-8 md:py-10", device === 'mobile' ? 'px-3' : 'px-4 md:px-6')}>
      ...
      <div className={cn(
        ...
        {displayItems.slice(0, itemCount).map((item) => {
          const discount = getDiscount(item.price, item.originalPrice);
          return (
            <div key={item.id} className="group relative rounded-2xl overflow-hidden cursor-pointer" style={imageAspectRatioStyle}>
              {item.image ? (
    ...
    <section className={cn("py-8 md:py-10", device === 'mobile' ? 'px-3' : 'px-4 md:px-6')}>
      ...
      <div className={cn(
        ...
        {displayItems.slice(0, itemCount).map((item) => {
          const discount = getDiscount(item.price, item.originalPrice);
          return (
            <div key={item.id} className="bg-white border border-slate-200 rounded-2xl p-3 flex flex-col group hover:shadow-lg hover:border-slate-300 transition-all cursor-pointer overflow-hidden">
              <div className="relative w-full rounded-xl overflow-hidden mb-3" style={{ ...imageAspectRatioStyle, backgroundColor: `${secondary}08` }}>
                {item.image ? (
            ...
            {visibleItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative bg-white overflow-hidden mb-3" style={imageAspectRatioStyle}>
                    {item.image ? (
                      <PreviewImage
                        src={item.image}
                        alt={item.name}
