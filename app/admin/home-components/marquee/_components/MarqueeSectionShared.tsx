...
import type { MarqueeBrandMode, MarqueeDirection, MarqueeItem, MarqueeScale, MarqueeSpeed, MarqueeStyle, MarqueeTextStyle } from '../_types';
...
// ── Layout 1: Ribbon ─────────────────────────────────────────────
function RibbonLayout({ items, tokens, duration, direction, pauseOnHover, scale }: LayoutProps) {
  const segments = buildSegments(items);
  const cfg = scaleConfig[scale];
  return (
    <div className={cfg.py} style={{ backgroundColor: tokens.ribbonBg }}>
...
// ── Layout 2: Gradient ───────────────────────────────────────────
function GradientLayout({ items, tokens, duration, direction, pauseOnHover, scale }: LayoutProps) {
  const segments = buildSegments(items);
  const cfg = scaleConfig[scale];
  return (
    <div
...
// ── Layout 3: Minimal ────────────────────────────────────────────
function MinimalLayout({ items, tokens, duration, direction, pauseOnHover, scale }: LayoutProps) {
  const segments = buildSegments(items);
  const cfg = scaleConfig[scale];
  return (
    <div className={cn(cfg.py, 'border-y')} style={{ backgroundColor: tokens.minimalBg, borderColor: tokens.minimalBorder }}>
      <MarqueeTrack direction={direction} duration={duration} pauseOnHover={pauseOnHover}>
...
// ── Layout 4: Dark ───────────────────────────────────────────────
function DarkLayout({ items, tokens, duration, direction, pauseOnHover, scale }: LayoutProps) {
  const segments = buildSegments(items);
  const cfg = scaleConfig[scale];
  return (
    <div className={cfg.py} style={{ backgroundColor: tokens.darkBg }}>
      <MarqueeTrack direction={direction} duration={duration} pauseOnHover={pauseOnHover}>
...
// ── Layout 5: Split ──────────────────────────────────────────────
function SplitLayout({ items, tokens, duration, direction, pauseOnHover, scale }: LayoutProps) {
  const segments = buildSegments(items);
...
function StripeLayout({ items, tokens, duration, direction, pauseOnHover, scale }: LayoutProps) {
  const segments = buildSegments(items);
  const cfg = scaleConfig[scale];
  return (
    <div className={cn(cfg.py, 'bg-white')}>
      <MarqueeTrack direction={direction} duration={duration} pauseOnHover={pauseOnHover}>
        <div className={cn('flex items-center px-4', cfg.fontSize, cfg.gap)}>
          {segments.map((seg, i) => (
            <React.Fragment key={i}>
...
export function MarqueeSectionShared(props: MarqueeSectionSharedProps) {
  const {
    ...
    tokens, title, subtitle, context, uppercase,
  ...
  const skipHeader = context === 'site';

  const layoutProps: LayoutProps = { items, tokens, duration, direction, pauseOnHover, scale: scale ?? 1 };
  ...
  return (
    <section style={{ fontFamily: "'Roboto Slab', serif", textTransform: uppercase ? 'uppercase' : 'none' }}>
      {!skipHeader && (
