...
import { getFooterLogoBackgroundClassName, getFooterLogoBackgroundStyle, getFooterLogoSize, getFooterMaxWidthClass } from '../_lib/constants';
...
export const FooterPreview = ({
  ...
  const logoInitial = (logoName || 'V').charAt(0);
  ...
  const renderLogoMark = (baseSize: number, imageClassName = 'object-contain', fallbackColor = colors.textOnPrimary) => {
    ...
    const content = config.logo
      ? <PreviewImage src={config.logo} alt={logoAlt} className={imageClassName} style={{ width: size, height: 'auto' }} />
  ...
  const previewVerticalPadding = getPreviewDeviceClass(device, {
    mobile: 'py-6',
    tablet: 'py-8',
    desktop: 'py-8',
  });
  ...
  const preview = () => {
    ...
    if (previewStyle === 'classic') {
      return (
        <footer className="w-full" style={{ backgroundColor: colors.classicBg }}>
          <div className={cn(maxWidthClass, 'mx-auto py-6', previewShellPadding)}>
                ...
                {config.showSocialLinks && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {socials.map((s, index) => {
                      ...
                      return (
                        <span key={`${s.id ?? 'social'}-${index}`} className="h-8 w-8 flex items-center justify-center rounded-full" style={{ backgroundColor: socialStyles.bg, color: socialStyles.color, ...(socialStyles.border ? { border: socialStyles.border } : {}) }}>
    ...
    if (previewStyle === 'modern') {
      ...
      return (
        <footer className="w-full relative" style={{ backgroundColor: colors.bg }}>
          ...
          <div className={cn(maxWidthClass, 'mx-auto relative', previewShellPadding, previewVerticalPadding)}>
    ...
    if (previewStyle === 'corporate') {
      return (
        <footer className={cn('w-full', previewVerticalPadding)} style={{ backgroundColor: colors.bg, borderTop: `1px solid ${colors.border}` }}>
                  ...
                  <>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {socials.map((s, index) => {
                        const socialStyles = resolveSocialStyles(s.platform, colors.socialBg, colors.socialText);
                        return (
                          <span key={`${s.id ?? 'social'}-${index}`} className="h-8 w-8 flex items-center justify-center rounded-lg" style={{ backgroundColor: socialStyles.bg, color: socialStyles.color, ...(socialStyles.border ? { border: socialStyles.border } : {}) }}>
                            <SocialIcon platform={s.platform} size={16} />
                          </span>
                        );
                      })}
                ...
                {config.showSocialLinks && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {socials.map((s, index) => {
                      const socialStyles = resolveSocialStyles(s.platform, colors.socialBg, colors.socialText);
                      return (
                        <span key={`${s.id ?? 'social'}-${index}`} className="h-7 w-7 flex items-center justify-center rounded-full" style={{ backgroundColor: socialStyles.bg, color: socialStyles.color, ...(socialStyles.border ? { border: socialStyles.border } : {}) }}>
    ...
    if (previewStyle === 'centered') {
      return (
        <footer className="w-full" style={{ backgroundColor: colors.magazineBg }}>
          <div className={cn(maxWidthClass, 'mx-auto', previewShellPadding, previewVerticalPadding)}>
    ...
    return (
      <footer className="w-full relative overflow-x-clip" style={{ backgroundColor: 'transparent' }}>
        ...
        <div className="relative" style={{ backgroundColor: colors.stackedTopBorder }}>
          ...
          <div className={cn(waveMaxWidthClass, 'mx-auto py-6 relative z-10', previewShellPadding)}>
            <div className={stackedGridClassName}>
              <div className="lg:col-span-3 md:col-span-3 space-y-2.5">
                {renderLogoMark(28, 'object-contain brightness-110', colors.stackedTextOnBg)}
