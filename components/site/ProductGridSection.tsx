...
export function ProductGridSection({ config, brandColor, secondary, title, snapshotComponentKey }: ProductGridSectionProps) {
        ...
        {products.slice(0, itemCount).map(p => {
          ...
          return (
            <Link
              key={p._id}
              href={getProductDetailHref(p)}
              className="bg-white border border-slate-200 rounded-2xl p-3 flex flex-col group hover:shadow-lg hover:border-slate-300 transition-all cursor-pointer overflow-hidden"
            >
  ...
  const renderTabbedSection = () => {
    // Tính contrast text color cho nền brandColor
    const tLum = (hex: string) => {
      const h = hex.replace('#', '');
    ...
    const textOnBrand = tLum(brandColor) > 0.4 ? '#1e293b' : '#ffffff';

    return (
      <section
        className="py-10 md:py-16 px-4 md:px-6 rounded-xl max-w-7xl mx-auto"
              ...
              <button
                ...
                style={
                  activeTabId === null
                    ...
                    : { border: `1px solid ${textOnBrand}4D`, color: textOnBrand }
                }
              >
                Tất cả
              </button>
  ...
  // Style 8: Storefront — brand header bar + white grid
      ...
      <section className="py-10 md:py-16 px-4 md:px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div
            ...
            style={{ backgroundColor: brandColor }}
          >
            <span className="font-bold text-sm whitespace-nowrap" style={{ color: (() => { const h = brandColor.replace('#', ''); const r = Number.parseInt(h.slice(0, 2), 16) / 255; const g = Number.parseInt(h.slice(2, 4), 16) / 255; const b = Number.parseInt(h.slice(4, 6), 16) / 255; const toL = (c: number) => (c <= 0.04045 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4); return (0.2126 * toL(r) + 0.7152 * toL(g) + 0.0722 * toL(b)) > 0.4 ? '#1e293b' : '#ffffff'; })() }}>Chọn danh mục</span>
            {displayTabs.length > 0 && (
              <>
                <button
                  type="button"
                  onClick={() => setActiveTabId(null)}
                  className={`px-3 py-1.5 rounded-md text-xs font-semibold whitespace-nowrap transition-colors bg-white text-slate-800 ${
                    activeTabId === null ? 'ring-2 ring-white/60' : ''
                  }`}
                >
                  Tất cả
                </button>
                {displayTabs.map(tab => (
                  <button
                    ...
                    onClick={() => setActiveTabId(tab.id === activeTabId ? null : tab.id)}
                    className={`px-3 py-1.5 rounded-md text-xs font-semibold whitespace-nowrap transition-colors bg-white text-slate-800 ${
                      activeTabId === tab.id ? 'ring-2 ring-white/60' : ''
                    }`}
                  >
                    {tab.name}
                  </button>
