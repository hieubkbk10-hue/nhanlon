...
import { getAPCATextColor, getBentoColors, getFadeColors, getFullscreenColors, getParallaxColors, getSliderColors, getSplitColors } from '@/app/admin/home-components/hero/_lib/colors';
...
export function HeroRuntimeSection({ config, brandColor, secondary, mode }: HomeComponentSectionProps) {
  ...
  const noBorderRadius = config.noBorderRadius === true;
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 flex justify-center gap-2 bg-gradient-to-t from-black/60 to-transparent z-20">
              {slides.map((slide, idx) => (
                <button key={idx} onClick={() => { setCurrentSlide(idx); }} className={`rounded overflow-hidden transition-all border-2 w-16 h-10 md:w-20 md:h-12 ${idx === currentSlide ? 'scale-105' : 'border-transparent opacity-70 hover:opacity-100'}`} style={idx === currentSlide ? { borderColor: fadeColors.thumbnailBorderActive } : { borderColor: fadeColors.thumbnailBorderInactive }}>
                  {slide.image ? (slide.mediaType === 'video' ? <video src={slide.image} className="w-full h-full object-cover" muted playsInline /> : <SiteImage src={slide.image} alt="" className="w-full h-full object-cover" loading="lazy" />) : renderPlaceholder(fadeColors.placeholderBg, fadeColors.placeholderIconColor, 18)}
                </button>
              ))}
            </div>
          )}
        ...
        <div className="max-h-[400px] md:max-h-[550px]">
          <div className="grid grid-cols-2 gap-2 md:hidden" style={{ height: '320px' }}>
            {bentoSlides.slice(0, 4).map((slide, idx) => (
              <a key={idx} href={slide.link || '#'} className={cn('relative overflow-hidden', !noBorderRadius && 'rounded-xl')}>
                {slide.image ? (
                  ...
                  <div className="w-full h-full relative">
                    ...
                    <div className="absolute inset-0 bg-black/20" />
                    <SiteImage src={slide.image} alt="" className="relative w-full h-full object-contain z-10" priority={idx === 0} loading={idx === 0 ? undefined : 'lazy'} sizes="50vw" />
                  </div>
                  )
          ...
          <div className="hidden md:grid grid-cols-4 grid-rows-2 gap-3" style={{ height: '500px' }}>
            <a href={bentoSlides[0]?.link || '#'} className={cn('col-span-2 row-span-2 relative overflow-hidden ring-2 ring-offset-1 ring-offset-slate-900', !noBorderRadius && 'rounded-2xl')} style={{ '--tw-ring-color': bentoColors.mainImageRing } as React.CSSProperties}>
              {bentoSlides[0]?.image ? (
            ...
            <a href={bentoSlides[1]?.link || '#'} className={cn('col-span-2 relative overflow-hidden', !noBorderRadius && 'rounded-2xl')}>
              {bentoSlides[1]?.image ? (
                ...
                <div className="w-full h-full relative">
                  ...
                  <div className="absolute inset-0 bg-black/20" />
                  <SiteImage src={bentoSlides[1].image} alt="" className="relative w-full h-full object-contain z-10" loading="lazy" sizes="25vw" />
                </div>
                )
              ) : renderPlaceholder(bentoPlaceholders[1], bentoColors.placeholderIcon, 22)}
            </a>
            <a href={bentoSlides[2]?.link || '#'} className={cn('relative overflow-hidden', !noBorderRadius && 'rounded-2xl')}>
              {bentoSlides[2]?.image ? (
                ...
                <div className="w-full h-full relative">
                  ...
                  <div className="absolute inset-0 bg-black/20" />
                  <SiteImage src={bentoSlides[2].image} alt="" className="relative w-full h-full object-contain z-10" loading="lazy" sizes="25vw" />
                </div>
                )
              ) : renderPlaceholder(bentoPlaceholders[2], bentoColors.placeholderIcon, 20)}
            </a>
            <a href={bentoSlides[3]?.link || '#'} className={cn('relative overflow-hidden', !noBorderRadius && 'rounded-2xl')}>
              {bentoSlides[3]?.image ? (
                ...
                <div className="w-full h-full relative">
                  ...
                  <div className="absolute inset-0 bg-black/20" />
                  <SiteImage src={bentoSlides[3].image} alt="" className="relative w-full h-full object-contain z-10" loading="lazy" sizes="25vw" />
                </div>
                )
        ...
        <div className="max-h-[400px] md:max-h-[550px]">
          <div className="flex flex-col gap-2 md:hidden" style={{ height: '420px' }}>
            {tripleSlides.slice(0, 3).map((slide, idx) => (
              <a key={idx} href={slide.link || '#'} className={cn('relative overflow-hidden flex-1', !noBorderRadius && 'rounded-xl')}>
                {slide.image ? (
                  slide.mediaType === 'video' ? (
                    <video src={slide.image} className="w-full h-full object-cover" muted loop autoPlay playsInline />
                  ) : (
                  <div className="w-full h-full relative">
                    <div className="absolute inset-0 scale-110" style={{ backgroundImage: `url(${slide.image})`, backgroundPosition: 'center', backgroundSize: 'cover', filter: 'blur(20px)' }} />
                    <div className="absolute inset-0 bg-black/20" />
                    <SiteImage src={slide.image} alt="" className="relative w-full h-full object-contain z-10" priority={idx === 0} loading={idx === 0 ? undefined : 'lazy'} sizes="100vw" />
                  </div>
                  )
          ...
          <div className="hidden md:grid grid-cols-3 gap-3" style={{ height: '500px' }}>
            {tripleSlides.map((slide, idx) => (
              <a key={idx} href={slide.link || '#'} className={cn('relative overflow-hidden', !noBorderRadius && 'rounded-2xl', idx === 0 && 'ring-2 ring-offset-1 ring-offset-slate-900')} style={idx === 0 ? { '--tw-ring-color': bentoColors.mainImageRing } as React.CSSProperties : undefined}>
                {slide.image ? (
                  slide.mediaType === 'video' ? (
                    <video src={slide.image} className="w-full h-full object-cover" muted loop autoPlay playsInline />
                  ) : (
                  <div className="w-full h-full relative">
                    ...
                    <div className="absolute inset-0 bg-black/20" />
                    <SiteImage src={slide.image} alt="" className="relative w-full h-full object-contain z-10" priority={idx === 0} loading={idx === 0 ? undefined : 'lazy'} sizes="33vw" />
                  </div>
                  )
      ...
      <section className="relative w-full bg-slate-900 overflow-hidden p-2 md:p-4">
        ...
        <div className="max-h-[400px] md:max-h-[550px]">
          ...
          <div className="hidden md:grid grid-cols-3 grid-rows-2 gap-3" style={{ height: '500px' }}>
            <a href={tripleSlides[0]?.link || '#'} className={cn('col-span-2 row-span-2 relative overflow-hidden ring-2 ring-offset-1 ring-offset-slate-900', !noBorderRadius && 'rounded-2xl')} style={{ '--tw-ring-color': bentoColors.mainImageRing } as React.CSSProperties}>
            ...
            {tripleSlides.slice(1, 3).map((slide, idx) => (
              <a key={idx} href={slide.link || '#'} className={cn('relative overflow-hidden', !noBorderRadius && 'rounded-2xl')}>
                {slide.image ? (
                  slide.mediaType === 'video' ? (
                    <video src={slide.image} className="w-full h-full object-cover" muted loop autoPlay playsInline />
                  ) : (
                  <div className="w-full h-full relative">
                    ...
                    <div className="absolute inset-0 bg-black/20" />
                    <SiteImage src={slide.image} alt="" className="relative w-full h-full object-contain z-10" loading="lazy" sizes="33vw" />
                  </div>
                  )
          ...
          {slides.map((slide, idx) => {
            ...
            return (
              <div key={idx} className={`absolute inset-0 transition-opacity duration-1000 ${idx === currentSlide ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                {slide.image ? renderHeroSlideContain(slide, { fit: 'cover', priority: idx === currentSlide, loading: shouldLoad ? 'eager' : 'lazy', overlay: showFullscreenContent ? <div className="absolute inset-0 z-20" style={{ background: `linear-gradient(to right, rgba(0,0,0,${(content.overlayOpacity ?? 50) / 100}), rgba(0,0,0,${(content.overlayOpacity ?? 50) / 250}), transparent)` }} /> : null }) : renderPlaceholder(fullscreenColors.placeholderBg, fullscreenColors.placeholderIcon)}
              </div>
            );
          })}
    ...
    <section className="relative w-full bg-slate-900 overflow-hidden">
      ...
      <div className="relative w-full h-[350px] md:h-[450px] lg:h-[550px]">
        ...
        <div className="absolute z-10 inset-x-4 md:inset-x-8 bottom-4 md:bottom-8 flex items-end">
          <div className="rounded-xl shadow-2xl p-4 md:p-6 max-w-lg" style={{ backgroundColor: parallaxColors.cardBg }}>
            ...
            <h3 className="text-lg md:text-xl font-bold" style={{ color: parallaxColors.headingText }}>{parseHighlightedHeading(content.heading ?? 'Tiêu đề nổi bật', content.highlightColor)}</h3>
