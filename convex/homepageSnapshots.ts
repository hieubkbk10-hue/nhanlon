...
const buildSnapshotDemoConfigs = async (
  ...
  const _getProductCategories = async () => {
    if (!_productCategories) _productCategories = await ctx.db.query('productCategories').withIndex('by_active', (q: any) => q.eq('active', true)).collect();
    return _productCategories!;
  };
  const _getServiceCategories = async () => {
    if (!_serviceCategories) _serviceCategories = await ctx.db.query('serviceCategories').withIndex('by_active', (q: any) => q.eq('active', true)).collect();
    return _serviceCategories!;
  };
  const getPostCategories = async () => {
    if (!_postCategories) _postCategories = await ctx.db.query('postCategories').withIndex('by_active', (q: any) => q.eq('active', true)).collect();
  ...
  handler: async (ctx) => {
    ...
    return rows
      ...
      .map((row) => {
        ...
        const thumbnails = extractSnapshotThumbnails(components, logo);
        // Build section summaries for richer descriptions
        const activeSections = components.filter((c) => c.active);
        const sectionTitles = activeSections.map((c) => c.title).filter(Boolean);
        return {
          _id: row._id,
          category: row.category || 'other',
          createdAt: row.createdAt,
          ...
          componentTypes: [...new Set(activeSections.map((c) => c.type))],
...
const rebuildHomeComponentStats = async (ctx: any) => {
  const [stats, components] = await Promise.all([
    ctx.db.query('homeComponentStats').take(5000),
